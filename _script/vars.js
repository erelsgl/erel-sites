...
/* _script/ckeditor/ckeditor_path */

/*
Set the path of ckeditor.
Must be included BEFORE ckeditor main script.
*/
window.CKEDITOR_BASEPATH = '/_script/ckeditor/';


/* _script/ckeditor/ckeditor */
...
(function(){function a(a,d){function e(a){return a||CKEDITOR.env.needsNbspFiller?new CKEDITOR.htmlParser.text("Â "):new CKEDITOR.htmlParser.element("br",{"data-cke-bogus":1})}function n(a,d){return function(f){if(f.type!=CKEDITOR.NODE_DOCUMENT_FRAGMENT){var n=[],q=b(f),g,A;if(q)for(w(q,1)&&n.push(q);q;)h(q)&&(g=c(q))&&w(g)&&((A=c(g))&&!h(A)?n.push(g):(e(p).insertAfter(g),g.remove())),q=q.previous;for(q=0;q<n.length;q++)n[q].remove();if(n=!a||!1!==("function"==typeof d?d(f):d))p||CKEDITOR.env.needsBrFiller||
...
{node:d.focusNode,offset:d.focusOffset}];d.anchorNode==c.$&&d.anchorOffset>f&&(g[0].offset-=f);d.focusNode==c.$&&d.focusOffset>f&&(g[1].offset-=f)}}c.setText(t(c.getText(),1));g&&(c=a.getDocument().$,d=c.getSelection(),c=c.createRange(),c.setStart(g[0].node,g[0].offset),c.collapse(!0),d.removeAllRanges(),d.addRange(c),d.extend(g[1].node,g[1].offset))}}function t(a,b){return b?a.replace(J,function(a,b){return b?"Â ":""}):a.replace(y,"")}function z(a,b){var c=b&&CKEDITOR.tools.htmlEncode(b)||"\x26nbsp;",
...
CKEDITOR.dtd.$empty))},(h=d.next())&&h.type==CKEDITOR.NODE_ELEMENT&&h.is("ul","ol")&&(CKEDITOR.env.needsBrFiller?g.createElement("br"):g.createText("Â ")).insertBefore(h)),c&&b.moveToElementEditStart(c);b.select();b.scrollIntoView()}}},enterBr:function(a,e,b,l){if(b=b||y(a)){var n=b.document,g=b.checkEndOfBlock(),f=new CKEDITOR.dom.elementPath(a.getSelection().getStartElement()),k=f.block,p=k&&f.block.getName();l||"li"!=p?(!l&&g&&w.test(p)?(g=k.getDirection())?(n=n.createElement("div"),n.setAttribute("dir",
...
CKEDITOR.env.needsNbspFiller&&7>=b.$.documentMode&&m.append(b.createText("Â ")),m.append(c.listNode),c=c.nextIndex;else if(-1==q.indent&&!d&&g){u[g.getName()]?(m=q.element.clone(!1,!0),n!=g.getDirection(1)&&m.setAttribute("dir",n)):m=new CKEDITOR.dom.documentFragment(b);var k=g.getDirection(1)!=n,w=q.element,B=w.getAttribute("class"),E=w.getAttribute("style"),J=m.type==CKEDITOR.NODE_DOCUMENT_FRAGMENT&&(f!=CKEDITOR.ENTER_BR||k||E||B),v,z=q.contents.length,x;for(g=0;g<z;g++)if(v=q.contents[g],I(v)&&
...
CKEDITOR.plugins.sourcearea={commands:{source:{modes:{wysiwyg:1,source:1},editorFocus:!1,readOnly:1,exec:function(c){"wysiwyg"==c.mode&&c.fire("saveSnapshot");c.getCommand("source").setState(CKEDITOR.TRISTATE_DISABLED);c.setMode("source"==c.mode?"wysiwyg":"source")},canUndo:!1}}};(function(){CKEDITOR.plugins.add("panel",{beforeInit:function(a){a.ui.addHandler(CKEDITOR.UI_PANEL,CKEDITOR.ui.panel.handler)}});CKEDITOR.UI_PANEL="panel";CKEDITOR.ui.panel=function(a,b){b&&CKEDITOR.tools.extend(this,b);CKEDITOR.tools.extend(this,{className:"",css:[]});this.id=CKEDITOR.tools.getNextId();this.document=a;this.isFramed=this.forceIFrame||this.css.length;this._={blocks:{}}};CKEDITOR.ui.panel.handler={create:function(a){return new CKEDITOR.ui.panel(a)}};var g=CKEDITOR.addTemplate("panel",
...
b[CKEDITOR.STYLE_OBJECT]||this.hideGroup(g["panelTitle"+String(CKEDITOR.STYLE_OBJECT)])},refresh:function(){var a=c.elementPath();if(a){for(var b in f)if(f[b].checkApplicable(a,c,c.activeFilter))return;this.setState(CKEDITOR.TRISTATE_DISABLED)}},reset:function(){f={};k=[]}})}})})();CKEDITOR.plugins.add("divarea",{afterInit:function(a){a.addMode("wysiwyg",function(c){var b=CKEDITOR.dom.element.createFromHtml('\x3cdiv class\x3d"cke_wysiwyg_div cke_reset cke_enable_context_menu" hidefocus\x3d"true"\x3e\x3c/div\x3e');a.ui.space("contents").append(b);b=a.editable(b);b.detach=CKEDITOR.tools.override(b.detach,function(a){return function(){a.apply(this,arguments);this.remove()}});a.setData(a.getData(1),c);a.fire("contentDom")})}});CKEDITOR.config.plugins='dialogui,dialog,about,basicstyles,notification,button,toolbar,clipboard,enterkey,entities,floatingspace,wysiwygarea,indent,indentlist,fakeobjects,link,list,undo,sourcearea,panel,floatpanel,listblock,richcombo,stylescombo,divarea';CKEDITOR.config.skin='moono-lisa';(function() {var setIcons = function(icons, strip) {var path = CKEDITOR.getUrl( 'plugins/' + strip );icons = icons.split( ',' );for ( var i = 0; i < icons.length; i++ )CKEDITOR.skin.icons[ icons[ i ] ] = { path: path, offset: -icons[ ++i ], bgsize : icons[ ++i ] };};if (CKEDITOR.env.hidpi) setIcons('about,0,,bold,24,,italic,48,,strike,72,,subscript,96,,superscript,120,,underline,144,,copy-rtl,168,,copy,192,,cut-rtl,216,,cut,240,,paste-rtl,264,,paste,288,,indent-rtl,312,,indent,336,,outdent-rtl,360,,outdent,384,,anchor-rtl,408,,anchor,432,,link,456,,unlink,480,,bulletedlist-rtl,504,,bulletedlist,528,,numberedlist-rtl,552,,numberedlist,576,,redo-rtl,600,,redo,624,,undo-rtl,648,,undo,672,,source-rtl,696,,source,720,','icons_hidpi.png');else setIcons('about,0,auto,bold,24,auto,italic,48,auto,strike,72,auto,subscript,96,auto,superscript,120,auto,underline,144,auto,copy-rtl,168,auto,copy,192,auto,cut-rtl,216,auto,cut,240,auto,paste-rtl,264,auto,paste,288,auto,indent-rtl,312,auto,indent,336,auto,outdent-rtl,360,auto,outdent,384,auto,anchor-rtl,408,auto,anchor,432,auto,link,456,auto,unlink,480,auto,bulletedlist-rtl,504,auto,bulletedlist,528,auto,numberedlist-rtl,552,auto,numberedlist,576,auto,redo-rtl,600,auto,redo,624,auto,undo-rtl,648,auto,undo,672,auto,source-rtl,696,auto,source,720,auto','icons.png');})();CKEDITOR.lang.languages={"en":1,"he":1};}());

/* _script/ckeditor/ckeditor_local */
...
var submitLabel = (lang=='en'? "submit": "ùìéçä");
var saveLabel =   (lang=='en'? "[s]ave changes": "ùîéøú äùéðåééí");
var undoLabel =   (lang=='en'? "return to the non-formal version": "äôåê àú äâéøñä äìà-øùîéú ìøùîéú");
var formatLabel =   (lang=='en'? "match destination formatting": "äúàí òéöåá éòã");
var versionsLabel =   (lang=='en'? "show former versions": "äöâ âéøñàåú ÷åãîåú");

var nameLabel = (lang=='en'? 'user name': 'ùí îùúîù');
var passwordLabel = (lang=='en'? 'password': 'ñéñîä');
var subjectLabel = (lang=='en'? 'subject': 'ðåùà');
var youForgotLabel = (lang=='en'? 'You forgot to fill in the ': 'ùëçú ìîìà '); // for ValidateNotEmpty
var authorLabel = (lang=='en'? 'Author - if different than username': 'îçáø/ú äîàîø');
...
bodyclassInput = '<select size="1" name="bodyclass" onchange="theAddForm.lang.selectedIndex=0; if (selectedIndex>0) {this.form.title.value=bodyclass_title(options[selectedIndex].text, options[selectedIndex].value); this.form.title.disabled=false;}">' + 
  '<option value="" selected="selected">' + (lang=='en'? '* add what? *': '* îä ìäåñéó? *') + 
'</select>';
...
function idFields(path_from_root_to_document,usernameHint,passwordHint,emailHint) {
	...
	'<tr>' + 
		...
		'<td><label>' + (lang=='en'? "<u>k</u>eep my details on this computer": "ùîåø àú äôøèéí ù<u>ì</u>é òì äîçùá äæä (òã ùàñâåø àú äçìåï)") + '</label>' + 
		'' + 
...
if (document.body.lang=='en') {
	...
	site_name='äàúø';
	usernameHint = '';
	passwordHint = '(àí àéï ìê ñéñîä, éù ìäùàéø øé÷. äùéðåééí ùìê ééëðñå ìâéøñä ìà øùîéú)';
	emailHint = '(ìà çåáä ìîìà)';
}

if (site=='tnk1') {
	site_name='àúø äðéååè áúðê';
	passwordHint = '(àí àéï ìê ñéñîä, éù ìëúåá àú 3 äàåúéåú äøàùåðåú áñôø öôðéä, áìé ðé÷åã. áîöá òøéëä, äùéðåééí ùìê ééëðñå ìâéøñä ìà øùîéú)';
	passwordIsRequired = true;
}
else if ((site=='nxt') || (site=='~nachat')) {
	site_name='àúø ðåòø çåáá úðê';
}
else if (site=='tokxot')
	site_name='äàúø ìîçàåú î÷åøéåú';
else if (site=='limudim')
	site_name='àúø äìéîåãéí ùì éòì ôø÷ì';
else if (site=='fuelcell')
	site_name='ôøåéé÷è îèåñ ììà èééñ îåðò áúàé ãì÷';
else if (site=='js')
	site_name='Javascript Algorithm Encyclopedia';
else if (site=='tryg')
	site_name='äàúø ìî÷åøéåú áîöååú';
else if (site=='erelsgl')
	site_name='äàúø ùì àøàì ñâì';
else if (site=='levisegal') {
	passwordIsRequired = true;
	site_name='àúø îùôçú äîáåøâø-ìåé-ñâì';
}
else if (site=='dugma') {
	usernameHint = '(áòøéëú äàúø ìãåâîä, éù ìäùúîù áùí "äðäìä")';
	passwordHint = '(áòøéëú äàúø ìãåâîä, éù ìäùúîù áñéñîä "äðäìä")';
	passwordIsRequired = true;
	site_name='àúø ìãåâîä';
}
else if (site=='demo') {
...
function replace_type_with_typeimage(bn) {
   if (site=='tnk1')
      return bn.
            replace(/ òì: /, ": ").
            replace(/ùéø:/, "<img title='ùéø' src="+path_from_document_to_site+"_themes/tw.gif>").
            replace(/ñøè:/, "<img title='ñøè' src="+path_from_document_to_site+"_themes/srt.gif>").
            replace(/ñéôåø:/, "<img title='ñéôåø' src="+path_from_document_to_site+"_themes/storybook.gif>").
            replace(/îùç÷:/, "<img title='ñéôåø' src="+path_from_document_to_site+"_themes/games.gif>").
            replace(/äáãìéí:/, "").
            replace(/îéãä èåáä:/, "").
            replace(/îéãä øòä:/, "").
            replace(/îàîø ãîéåðé:/, "<img title='ãéîéåðé' src="+path_from_document_to_site+"_themes/ein_knisa.gif>îàîø ãéîéåðé:").
            replace(/îàîø:/, "").
            replace(/úøâåîéí:/, "").
            replace(/àåñó:/, "").
            replace(/ëìì:/, "").
            replace(/îàîø ìà âîåø:/, "<img title='ìà âîåø' src="+path_from_document_to_site+"_themes/jtil.gif> ").
            replace(/ùàìä:/, "<img title='ùàìä' src="+path_from_document_to_site+"_themes/jtil.gif> ").
            replace(/îöååú:/, "<img title='îöåä' src="+path_from_document_to_site+"_themes/mcwa.gif> ").
            replace(/îáðä:/, "");
   else
      return bn;
}
   ...
   for (i=0; i<theBnimArray.length; ++i) {
      ...
      if (regexp1.test(bn) || regexp2.test(bn)) {
        if (/href/.test(bn) || !/: -/.test(bn)) {
          ...
          if (!/img/i.test(bn)) {
            theText += "<li>";
            bn = bn.replace(/öéåø/, "<img title='öéåø' src="+path_from_document_to_site+"_themes/cyur.gif>öéåø");
          }
...
function ktov_bnim_tnk() {
	...
	document.write (
			"</ol>");
					ktov_bnim('<h2>îàîøéí</h2> <ol class="awsp_mpwrt"> ', '(úøâåîéí|àúø|äáãìéí|îàîø|îáðä|îöâú|÷åáõ|ëìì|ñôø|äâãøä):', ' </ol>');
					ktov_bnim('<h2>îàîøéí ãîéåðééí åàîðåúééí</h2> <ol class="awsp_mpwrt"> ', '(îàîø ãîéåðé|ñéôåø|ùéø|öéåø|ñøè):', ' </ol>');
					ktov_bnim('<h2>îùç÷éí åôòéìåéåú</h2> <ol class="awsp_mpwrt"> ', 'îùç÷:', ' </ol>');
					ktov_bnim('<h2>ùàìåú åîàîøéí ìà âîåøéí</h2> <ol class="awsp_mpwrt"> ', '(îàîø ìà âîåø|îàîø ùìà ôåøñí|ùàìä):', ' </ol>');
...
function ktov_bnim_jorj() {
	...
	document.write (
		"<div class='tt_jorjim'>");
			ktov_bnim('<ul> ', '(àùîæ|ñùîæ|ùæîù):', ' </ul>');
			ktov_bnim('<ul> ', '([^<>]*ùåøù[^<>]*):', ' </ul>');
			ktov_bnim('<h3>ùåøùéí ðåáòéí</h3> <ul> ', 'ùð:', ' </ul>');
	document.write (
		"</div>" + 

		"<table class='milim_mjorj'><thead><col><col></thead><tbody>" + 
		"<tr><td>");
			ktov_bnim('<h3>ùîåú ùì ä\'</h3> <ul> ', 'ùí1:', ' </ul>');
			ktov_bnim('<h3>úàøéí åùîåú îúåàøéí</h3> <ul> ', 'úåàø:', ' </ul>');
			ktov_bnim('<h3>àåúéåú</h3> <ul> ', 'àåú:', ' </ul>');
			ktov_bnim('<h3>ôòìéí</h3> <ul> ', 'ôåòì:', ' </ul>');
			ktov_bnim('<h3>îéìéí ëììéåú</h3> <ul> ', 'îéìä:', ' </ul>');
			ktov_bnim('', 'öé[^<>]*å[^<>]*:', '');
	document.write (
		"<td>");
			ktov_bnim('<h3>çéåú åöîçéí</h3> <ul> ', 'çéä:', ' </ul>');
			ktov_bnim('<h3>òöîéí îåçùééí</h3> <ul> ', 'òöí:', ' </ul>');
			ktov_bnim('<h3>ëìéí îìàëåúééí</h3> <ul> ', 'ëìé:', ' </ul>');
			ktov_bnim('<h3>ùîåú îåôùèéí</h3> <ul> ', 'îåôùè:', ' </ul>');
	document.write (
		"</table>" + 
		"<table class='mamrim_mjorj'><thead><col><col></thead><tbody>" + 
		"<tr><td>");
			ktov_bnim('<h2>äâãøåú</h2> <ul> ', 'äâãøä:', ' </ul>');
			ktov_bnim('<h2>äáãìéí</h2> <ul> ', 'äáãì:', ' </ul>');
	document.write (
		"<td>");
			ktov_bnim('<h2>ôéøåùé ôñå÷éí</h2> <ul> ', 'úåëï1:', ' </ul>');
			ktov_bnim('<h2>îàîøéí</h2> <ul> ', '(îàîø|ëìì|îàîø ìà âîåø|îàîø ùìà ôåøñí|îàîø ãîéåðé):', ' </ul>');
	document.write (
		"</table>");
			ktov_bnim('<h3 id=bituy>áéèåééí</h3> <ul> ', '[^<>]*áéèåé[^<>]*:', ' </ul>');
	document.write ("<br style='clear:all' />");
}


function ktov_bnim_mcwa() {
	ktov_bnim("<div class='pswqym'> <h2>ôñå÷éí</h2> <ul> ", "ôñå÷:", " </ul> </div>");
	ktov_bnim("<div class='femym'> <h2>èòîéí</h2> <ul> ", "èòí:", " </ul> </div>");
	ktov_bnim("<div class='hgdrwt'> <h2>äâãøåú åäìëåú</h2> <ul> ", "(äâãøä|äìëåú):", " </ul> </div>");
	ktov_bnim("<div class='drkym'> <ul> ", "ãøê:", " </ul> </div>");
	ktov_bnim("<div class='sywwgym'> <h2>ñéååâ äîöåä (âåøîéí ùäîöåä úìåéä áäí)</h2> <ul> ", "ñéååâ:", " </ul> </div>");
}
...
function ktov_bnim_mcwot(tdirut) {
	...
	document.write(
		...
		"<td class='drkym'>");
		ktov_bnim("<h2>ãøëéí ì÷éåí äîöååú áéîéðå</h2> <ol class='TableSublists'>",
			"ãøê:"," </ol>");
	...
	if (tdirut == 'erua') {
		ktov_bnim("<h2>îöååú ùéù ì÷ééí ëù÷åøä àøåò æä</h2> <ol class='TableSublists'>", 
			"(îöååú_øâò|îöåä|îöååú|îöååú_àøåò|îöååú_æîï):", "</ol>");
		ktov_bnim("<h2>îöååú ùúìåéåú áàãí îñåééí àå áçôõ îñåééí</h2> <ol class='TableSublists'>",
			"(îöååú_àãí|îöååú_òöí|îöååú_î÷åí):","</ol>");
		ktov_bnim("<h2>îöååú ùéù ì÷ééí ëùøåöéí ìòùåú ôòåìä îñåééîú</h2> <ol class='TableSublists'>",
			"îöååú_øöåï:","</ol>");
		ktov_bnim("<h2>ðåùàéí ÷øåáéí</h2> <ol class='TableSublists'>",
			"(îöååú_òøê|îöååú_çåáú):", " </ol>");
	}
	else if (tdirut == 'rcon') {
		ktov_bnim("<h2>îöååú ùéù ì÷ééí ëùøåöéí ìòùåú ôòåìä æå</h2> <ol class='TableSublists'>", 
			"(îöååú_øâò|îöåä|îöååú|îöååú_øöåï):", "</ol>");
		ktov_bnim("<h2>îöååú ùúìåéåú áàãí îñåééí àå áçôõ îñåééí</h2> <ol class='TableSublists'>",
			"(îöååú_àãí|îöååú_òöí|îöååú_î÷åí):","</ol>");
		ktov_bnim("<h2>îöååú ùéù ì÷ééí ëù÷åøä àøåò îñåééí</h2> <ol class='TableSublists'>",
			"(îöååú_àøåò|îöååú_æîï):", "</ol>");
		ktov_bnim("<h2>ðåùàéí ÷øåáéí</h2> <ol class='TableSublists'>",
			"(îöååú_òøê|îöååú_çåáú):", " </ol>");
	}
	else if (tdirut == 'ecm') {
		ktov_bnim("<h2>îöååú ùöøéê ì÷ééí á÷ùø ìàãí æä / çôõ æä</h2> <ol class='TableSublists'>", 
			"(îöååú_øâò|îöåä|îöååú|îöååú_òöí|îöååú_àãí|îöååú_î÷åí):", "</ol>");
		ktov_bnim("<h2>îöååú ùéù ì÷ééí ëùøåöéí ìòùåú ôòåìä îñåééîú</h2> <ol class='TableSublists'>",
			"îöååú_øöåï:","</ol>");
		ktov_bnim("<h2>îöååú ùéù ì÷ééí ëù÷åøä àøåò îñåééí</h2> <ol class='TableSublists'>",
			"(îöååú_àøåò|îöååú_æîï):", "</ol>");
		...
		ktov_bnim("<h2>îöååú åèòîéí</h2> <ol class='TableSublists'>", 
			"(îöååú_øâò|îöåä|îöååú|îöååú_òöí|îöååú_àãí|îöååú_î÷åí|îöååú_àøåò|îöååú_æîï|îöååú_øöåï|îöååú_çåáú):", "</ol>");
		ktov_bnim("<h2>îöååú àçøåú áðåùàéí ÷øåáéí</h2> <ol class='TableSublists'>",
			"îöååú_òøê:", " </ol>");
	}
	else /*if (tdirut == 'mcwot1')*/ {
		ktov_bnim("<ol class='TableSublists'>", 
			"(îöååú_øâò|îöåä|îöååú|îöååú_òöí|îöååú_àãí|îöååú_î÷åí|îöååú_àøåò|îöååú_æîï|îöååú_øöåï|îöååú_çåáú|îöååú_òøê):", "</ol>");
	}
	...
	document.write(
		...
		ktov_bnim_axrim("<h2>îàîøéí, ãéåðéí å÷éùåøéí</h2> <ol class='TableSublists'>",
			"</ol>");
	document.write(
		"</table>");
}
...
function table_link_item(table_name, table_description) {
	return "<li>" +
		"<b>" + table_name + "</b> (" +
		"<a href='" + magrim_root + table_name + ".sql'>äâãøä</a>, " +
		"<a href='" + magrim_root + table_name + ".txt'>úåëï</a>): " +
		table_description +
		"</li>\n";
}
...
function mdb_table_link_item(table_name, table_description) {
	return "<li>" +
		"<b>" + table_name + "</b> (" +
		"<a href='" + path_from_document_to_root + "tnk1/_mdb/" + table_name +  ".txt'>úåëï</a>; <small>äùåøä äøàùåðä îëéìä àú ùîåú äùãåú</small>): " +
		table_description +
		"</li>\n";
}
...
function show_table_warning() {
	document.write("<li>æäéøåú! çì÷ îä÷áöéí âãåìéí îàã (áñáéáåú 2 î&quot;á). ìà îåîìõ ìä÷ìé÷ òìéäí òí äëôúåø äùîàìé, ëé æä éâøåí ìäí ìäéôúç áçìåï äãôãôï åìú÷åò àåúå. òãéó ìä÷ìé÷ òìéäí òí äëôúåø äéîðé åìùîåø àåúí òì äîçùá ùìëí. ìàçø îëï ðéúï ìòøåê àåúí áòæøú &quot;à÷ñì&quot;." + 
	"</li>\n" + 
	"<li>çì÷ îä÷áöéí äí á÷éãåã çìåðåú 1255, åçì÷ á÷éãåã utf-8. àí àúí ôåúçéí ÷åáõ åøåàéí ñéîðéí îåæøéí, ðñå ìäçìéó àú ä÷éãåã." + 
	"</li>\n");
}
...
var letters1 = 'àáâãäåæçèé'.split('');
var letters2 = 'éëìîðñòôö÷'.split('');
var letters3 = '÷øùú'.split('');
...
function setLanguage(theNewLanguage) {
	...
	newTguvotHeading1 = "<span lang='he' xml:lang='he'>úâåáåú</span> <span lang='en' xml:lang='en'>Replies</span>";
	newTguvotHeading2 = "<span lang='he' xml:lang='he'>úú-ðåùàéí</span> <span lang='en' xml:lang='en'>Subjects</span>";
	if (theNewLanguage == '^he') {
		...
		if (theTguvotHeading) {
			theTguvotHeading.innerHTML = theTguvotHeading.innerHTML.
				replace(/^úâåáåú ?$/,newTguvotHeading1).
				replace(/^úú-ðåùàéí ?$/,newTguvotHeading2);
		}
	}
}
...
function kotrt() {
	...
	if (!is_local) {
		if (/öçåø/.test(theAuthor) || /àáéúø/.test(theAuthor) || /äåôø/.test(theAuthor))  {
			google_ad_client = "pub-4131841895603404";
			/* 728x90, ðåöø 11/05/08 */
	...
	var theText="";
	if (theReceiver=='ñâìåú îùìé') {
		theText += ("<p class='sgulot_logo'>" + 
			"<a href='https://www.lulu.com/spotlight/erel'>" +
			"<img src='/tnk1/_themes/sgulot.png' alt='ñâåìåú îùìé' title='îúåê äñôø ñâåìåú îùìé'/>" +
			"</a>" +
			"</p>");
	}
	...
	if (site=="tnk1") {
		theText += "<a href='"+path_from_document_to_root+site+"/index.html'>"+"<img class='logo' title='çæøä ìãó äøàùé' alt='çæøä ìãó äøàùé' src='" + path_from_document_to_root + site + "/_themes/logo_small.gif' /></a>";
	...
	if (theAuthor.length>1 && !/erelsgl|brain/.test(path_from_root_to_document) && !(site == "horsha")) {
		if ((theAuthor=="àøàì" || theAuthor=="àøàì ñâì"))
			authorForTitle = bilingual(lang,theLang2,"span",authorLink("àøàì ñâì äìåé, ììà äñîëä ìôñé÷ú äìëä, ìà òöáðé, ìà òéúåðàé", "erelvgalya+tnk1","gmail.com"),"Erel Segal");
		else if (/erel/i.test(theAuthor)) 
			authorForTitle = bilingual(lang,theLang2,"span","àøàì ñâì","Erel Segal");
		else if ("øòðï" == theAuthor || /øòðï àééëìø/.test(theAuthor))
			authorForTitle = authorLink("øòðï àééëìø", "raananyosef","yahoo.com");
		else if ("ãåã ÷" == theAuthor || "ãåã ÷éååéúé" == theAuthor) 
			authorForTitle = "òì-ôé ùéòåøéí ùì ôøåô' ÷éøùðáàåí îäîøëæ äáéðúçåîé";
		else if ("öçåø" == theAuthor)
			authorForTitle = authorLink("öçåø", "the.tzahor","gmail.com");
		else if ("àáéâéì âåèìéá" == theAuthor)
			authorForTitle = authorLink("àáéâéì âåèìéá", "avigaelgotlib", "walla.com")
		else if ("éòì ôø÷ì" == theAuthor)
			authorForTitle = authorLink("éòì ôø÷ì", "syaelp","t2.technion.ac.il");
		else if (/îùä ãéï/.test(theAuthor))
			authorForTitle = authorLink("îåøä ìúåøä îùä ãéï", "DAIAN1","013.NET");
		else if (/àáéäå/.test(theAuthor) && /àéúï/.test(theAuthor))
			authorForTitle = authorLink("ùìç: àáéäå àéúï çãã", "eitan-avioh","iec.co.il");
		else if (/éøåï úàðä/.test(theAuthor))
			authorForTitle = authorLink("éøåï úàðä", "yaront","kibbutzmerav.co.il");
		else if (/îùä áìéãùèééï/.test(theAuthor))
			authorForTitle = authorLink("îùä áìéãùèééï", "moblid","gmail.com");
		else if (/àáøäí áï òæøà/.test(theAuthor))
			authorForTitle = authorLink("ã'ø àáøäí áï òæøà", "bezra","inter.net.il");
		else if (/çâé øôé/.test(theAuthor) || /øôàì áø àùø çâáé/.test(theAuthor) )
			authorForTitle = authorLink("øôàì áø àùø çâáé - çâé øôé", "Hagay_r","tahal.com");
		else if ("àáéúø"==theAuthor || "àáéúø ëäï"==theAuthor )
			authorForTitle = authorLink("<a href='/tnk1/find.php?q=àáéúø ëäï'>àáéúø ëäï</a>", "evycohen","walla.com");
			//authorForTitle = authorLink("<a href='http://www.facebook.com/evycohen'>àáéúø ëäï</a>", "evycohen","walla.com");
		else if (/àìéäå îàìé/.test(theAuthor))
			authorForTitle = ("äøá àìéäå îàìé <p style='font-weight:bold; color:#080'>îåúø ìäòúé÷ ùìà ìîèøåú îñçøéåú</p>");
		else if (/éåñó ôøéàì/.test(theAuthor))
			authorForTitle = authorLink ("éåñó ôøéàì","priel9","biu.013.net.il");
		else if (/øàåáï æéìáøîï/.test(theAuthor))
			authorForTitle = authorLink ("øàåáï æéìáøîï","reuven49","walla.com");
		else
			authorForTitle = theAuthor;

		theText += (
			bilingual(lang,theLang2, "span", "îàú: ", "By: ") +
			authorForTitle);
		
		if (document.body.className == "dmwt") {
			theText += ("<br />" + "îéîåï: " + "àøàì");
		}
	}

	if (theReceiver.length>0) {
		if (theReceiver=='ñâìåú îùìé') {
			theText += ("<br />" + "îúåê äñôø " + 
				"<a href='https://www.lulu.com/spotlight/erel'>ñÀâËìÌåÉú îÄùÑÀìÅé</a>");
		} else {
			theReceiver = theReceiver.replace(/ëôéú( ùì ðç.?ú)?/, "ëôéú ùì <a href='nachat.ipaper.co.il'>ðç\"ú</a>");
			if (!/ôå\'\'ñ/.test(theReceiver) && !/ðð\'\'ð/.test(theReceiver) )
				theText += ("<br />" + 
					bilingual(lang,theLang2, "span", "àì: ", "To: ") +
					theReceiver);
		}
	}

	if (/jdl.*Mefar/.test(path_from_root_to_document))   theText += ('<p>îäãåøä àì÷èøåðéú æå ùì ôéøåùé ùã"ì îå÷ãùú ìòéìåé ðùîúå ùì ã"ø ùìîä à. ðëåï (ìéååøðå, úøñ"å - éøåùìéí, úùì"ã), âåàìí åîåöéàí ìàåø ùì ôéøåùé ùã"ì áéîéðå. ëì äæëåéåú ùîåøåú ìîùôçä.');
...
function linkToFormalVersion() {
	return  " " + 
		...
		(lang=='en'? "Click here to view the formal version of this document": "ìçöå ëàï ëãé ìøàåú àú äâéøñä äøùîéú ùì äãó") + 
		"</a> ";
}
	...
	if (/http:/.test(document.URL) && currentVersionIsFormal() && !/compact|csv_ezor[.]p/.test(document.URL)) {
		...
		*/
		...
		if (currentVersionIsFormal()) {
			document.write(lang=='en'? 'This is the <b>formal version</b> of the document, last updated at ': 'æåäé <b>äâøñä äøùîéú</b> ùì äãó, ùòåãëðä ìàçøåðä á-');
			document.write(document.lastModified + '. ');
		} else {
			document.write
			(lang=='en'? 'This is a non-formal version of the document, created ': 'æåäé âéøñä ìà øùîéú ùì äãó, ùðåöøä ' );
			document.write(document.lastModified + '. ');
...
function txtit() {
	...
	if (/àøàì/.test(theAuthor))
		theText += ("<br />ðéúï ìäòúé÷ åìäùúîù áãó æä áàåôï çåôùé, òì-ôé úðàé äøùéåï ùì ÷øï äúåëðä äçåôùéú (ùá<a href='"+path_from_document_to_site+"../rjyon.html'>÷éùåø</a>)");
	if (!/compact/.test(location.href)) {
		if (theAuthor.length>0 || (theReceiver.length>0 && !/[@]/.test(theReceiver)) )
			theText += (lang=='en'? "<br />more articles:&nbsp;&nbsp;&nbsp;" :"<br />÷øàå îàîøéí ðåñôéí:&nbsp;&nbsp;&nbsp; ");
		if (theAuthor.length>0) {
			theAuthorWithoutParens = theAuthor.
				replace(/ ?\(.*\)/,'').
				replace(/^ò["]?ô /, '');
			//theText += linkToGoogleSearchResults('"îàú ' + theAuthorWithoutParens + '"') + '&nbsp;&nbsp;&nbsp; ';
			theText += 'îàú ' + linkToOurSearchResults(theAuthorWithoutParens) + '&nbsp;&nbsp;&nbsp; ';
			if (/ /.test(theAuthorWithoutParens)) {
				theAuthorFirstWord = theAuthorWithoutParens.replace(/ .*$/,'');
				//theText += linkToGoogleSearchResults('"îàú ' + theAuthorFirstWord + '"') + '&nbsp;&nbsp;&nbsp; ';
				theText += 'îàú ' + linkToOurSearchResults(theAuthorFirstWord) + '&nbsp;&nbsp;&nbsp; ';
			}
		}
		if (theReceiver.length>0 && !/[@]/.test(theReceiver)) {
			theReceiverWithoutParens = theReceiver.replace(/ ?\(.*\)/,'');
			theText += linkToGoogleSearchResults('"àì ' + theReceiverWithoutParens + '"') + '&nbsp;&nbsp;&nbsp; ';
			if (/ /.test(theReceiverWithoutParens)) {
				theReceiverFirstWord = theReceiverWithoutParens.replace(/ .*$/,'');
				theText += linkToGoogleSearchResults('"àì ' + theReceiverFirstWord + '"') + '&nbsp;&nbsp;&nbsp; ';
			}
		}
		...
		/*	
		theText += " <br />" + (lang=='en'? "Document validity check: ": "áãé÷ú ú÷éðåú äîñîê: ") + 
	...
	theText += (" <br /> " + 
	(lang=='en'? "The internet address of this document is": "ãó æä ðîöà áøùú áëúåáú") +
...
function standardizeHTMLTokn(theHTML) {
	...
	/* fix span direction (for bidi text) */
		...
		replace(/(<li[^>]*>)([^à-ú]*)(<span[^>]*)dir=\"?...\"?([^>]*>)/ig,"$1$2$3$4").
...
function standardizeHTML() {
	if (theTokn && theTguvot) {
		...
		alert('äúâìúä ú÷ìä áîáðä äîñîê. ðà ìäåãéò ìîðäì äàúø!');
	}
}
...
function showAdd(theForm) {
	...
	i = 0;
	if (theForm == 'addtext') {
    	theAddForm.bodyclass.options[i++] = new Option( (lang=='en'? '* add what? *': '* îä ìäåñéó? *'), '');
		theAddForm.bodyclass.options[i++] = new Option( (lang=='en'? 'reply': 'úâåáä'), 'tguva');
		theAddForm.bodyclass.options[i++] = new Option( (lang=='en'? 'new article': 'îàîø çãù'), 'newarticle');
		theAddForm.bodyclass.options[i++] = new Option( (lang=='en'? 'new subject': 'ðåùà çãù'), 'newsubject');
		theAddForm.bodyclass.options[i++] = new Option( (lang=='en'? 'question': 'ùàìä'), 'la_gmwr');
		if (/tnk1.ljon.jorj/.test(path_from_root_to_document)) {
			theAddForm.bodyclass.options[i++] = new Option('äâãøä', 'hgdrh');
			theAddForm.bodyclass.options[i++] = new Option('äáãì', 'hbdl');
		}
		if (/tnk1/.test(path_from_root_to_document)) {
			theAddForm.bodyclass.options[i++] = new Option('îàîø ãîéåðé', 'hpwk');
			theAddForm.bodyclass.options[i++] = new Option('ùéø', 'jyr');
		}
		if (/tryg.mcwa/.test(path_from_root_to_document)) {
			theAddForm.bodyclass.options[i++] = new Option('äâãøä', 'hgdrh');
			theAddForm.bodyclass.options[i++] = new Option('èòí', 'fem');
		}

	}
	else if (theForm == 'addfile') {
    	theAddForm.bodyclass.options[i++] = new Option( (lang=='en'? '* add what? *': '* îä ìäåñéó? *'), '');
        theAddForm.bodyclass.options[i++] = new Option( (lang=='en'? 'image': 'öéåø àå öéìåí'), 'image');
        theAddForm.bodyclass.options[i++] = new Option( (lang=='en'? 'tab-seperated text': 'èáìä á÷åáõ è÷ñè'), 'magr');
        theAddForm.bodyclass.options[i++] = new Option( (lang=='en'? 'other file': '÷åáõ îñåâ àçø'), 'file');
	}
	else {
        theAddForm.bodyclass.options[i++] = new Option( (lang=='en'? 'Title of article in link': 'ëåúøú äîàîø á÷éùåø'), 'externallink');
	}
}
...
help_he = '<li>ëãé ìîçå÷ àú äîñîê, äëðéñå àú äè÷ñè "ìîçé÷ä" áî÷åí ëìùäå áîñîê.'+
	'<li>ðéúï ìäùúîù áî÷ùé-òøéëä: <STRONG>Ctrl+B ìäãâùä</STRONG>, <EM>Ctrl+I ìäèéä</EM>, <U>Ctrl+U ì÷å-úçúé</U>, <a href="javascript:alert(\'Ctrl+K = äåñôú ÷éùåø\')">Ctrl+K ìäåñôú ÷éùåø</a>' +
	'<li>ðéúï ìòøåê á"ååøã" àå áúåëðä àçøú, åàæ ìäòúé÷ åìäãáé÷ ëàï.' + 
	'<li>ìäåñôú öéåø àå öéìåí éù ùúé àôùøåéåú: <ul>' + 
	'<li>àí äöéåø ðîöà áàéðèøðè - ôùåè ñîðå àåúå áòæøú äòëáø, äòúé÷å (Ctrl+C) åäãáé÷å (Ctrl+V). æäéøåú - àì úôâòå áæëåéåú éåöøéí!' + 
	'<li>àí äöéåø ðîöà òì äîçùá ùìëí - ìçöå òì "äåñó ÷åáõ" åäåñéôå àú äöéåø ùìëí ìàúø, åàæ úåëìå ìñîï àåúå ìäòúé÷ åìäãáé÷ áàåúå àåôï.</ul>' + 
	'ìàçø ùîãáé÷éí àú äöéåø áîñîê, ðéúï ìä÷ìé÷ òìéå åàæ ìùðåú àú âåãìå ò"é îùéëú äøéáåòéí äìáðéí ä÷èðéí ùî÷éôéí àåúå.<ul>'; + 
	'';
...
function submitOrDebug(text) {
	...
	if (text == null) {
		alert ("äè÷ñè øé÷! àðà äåãéòå ìîðäì äàúø!");
		return false;
	}
	if (text.length == 0) {
		alert ("äè÷ñè äùúáù! àðà äåãéòå ìîðäì äàúø!");
		return false;
	}
...
function tguva() {
	...
	if (/áøåøéä/.test(theAuthor)) {
		document.write('<div style="font-style:italic">ðéúï ìéöåø ÷ùø òí äëåúáú ãøê <a href="http://www.mycreation.co.il" target="_blank">äàúø ùìä</a>.</div>');
	} else {
		if (!theTvnit.length)
	...
	if (theTvnit == "0") {
		teur1 = "ìçöå ëàï ëãé ìäåñéó ìàúø ãó çãù ùäâåìùéí éåëìå ìäâéò àìéå îäãó äøàùé.";
	} else if (theTvnit.length>0) {
		teur1 = "àí ëúáúí îàîø ù÷ùåø ì"+document.title+", àúí îåæîðéí ìùéí àåúå ëàï; àí éù ìëí ùàìä òì ãó æä (ìà ùàìä äìëúéú) àúí îåæîðéí ìùàåì àåúä ëàï";
	} else {
		teur1 = "àí éù ìëí úâåáä òì "+document.title+", àúí îåæîðéí ìùéí àåúä ëàï; àí éù ìëí ùàìä òì ãó æä (ìà ùàìä äìëúéú) àúí îåæîðéí ìùàåì àåúä ëàï";
	}
	...
	document.write(buttonGroup(
	   ...
	   bilingual(lang, theLang2, "span", "äåñó ãó çãù àå úâåáä", "Add a new page or reply"),
	   (lang=='en'? '': teur1),

	   (theTvnit.length>0? "showAdd('addlink');theAddForm.qijur.disabled=false;theAddForm.jm_qovc_al_hlqox.disabled=true;": ""),
	   bilingual(lang, theLang2, "span", "äåñó ÷éùåø", "add hyperlink"),
	   (lang=='en'? '': 'àí îöàúí áàéðèøðè îàîø àå àúø ù÷ùåø ì'+document.title+', àúí îåæîðéí ìùéí ëàï ÷éùåø. <small>àôùø ìäåñéó âí ÷éùåø ìîàîø ùðîöà áàúø æä.</small>'),

	   (theTvnit.length>0? "showAdd('addfile');theAddForm.qijur.disabled=true;theAddForm.jm_qovc_al_hlqox.disabled=false;": ""),
	   bilingual(lang, theLang2, "span", "äåñó öéåø, öéìåí àå ÷åáõ àçø", "add image or another file"),
	   (lang=='en'? '': 'àí éù ìëí ÷åáõ ù÷ùåø ì'+document.title+' (öéåø, îöâú, ùéø...) àúí îåæîðéí ìùéí àåúå ëàï'),

	   (documentHasEditableParts? "showEdit('editwithbuttons')": ""),
	   bilingual(lang, theLang2, "span", "òöá ãó æä", "edit with style"),
	   (lang=='en'? '': 'àí àúí øåöéí ìäåñéó öáòéí, ëåúøåú, øùéîåú àå òéöåáéí àçøéí - ìçöå ëàï')
	) + "\n\n");
	...
	if (theTvnit.length>0) {
		...
		if (theTvnit=='0') {
			...
			document.write(
				"<div id='addtext'>\n" + 
					(lang=='en'? '': '<p><B>úðàé ùéîåù</B>: äèåôñ îéåòã ìúâåáåú òðééðéåú áìáã. áéèåéé ìòâ åæìæåì ééîç÷å ììà àæäøä. ìçéöä òì ëôúåø "ùìéçä" îäååä äñëîä ìúðàé æä.') + 
					(lang=='en'? '<h3>Content (text and hyperlinks)</h3>': '<h3>úåëï (è÷ñè å÷éùåøéí):</h3>') + 
		...
		document.write(
			"<div id='addfile'>\n"+
				(lang=='en'? "<h3>A file from your computer</h3>": "<h3>÷åáõ îäîçùá ùìëí</h3>")+
				...
				(lang=='en'? 'replace file if it exists': 'äçìó ÷åáõ ÷ééí áùí æä')+
			"</div>\n"+

			"<div id='addlink'>\n"+
				"<p><b>ä÷éùåø:</b> <input type='text' name='qijur' dir='ltr' value='http://' size='90' style='font-size:10px'/></p>\n"+
			"</div>\n"+
			...
			"äúâåáä úåôéò áàúø úåê îñôø ã÷åú. àðà äúàæøå áñáìðåú! àí äúâåáä ìà ðùìçú - àðà ëúáå ãåàì ìîðäì äàúø")+
	...
	if (documentHasEditableParts) {
		document.write("<div id='editwithbuttons'>" + 
			(lang=='en'? '<h3>Edit</h3>': '<h3>òøéëä</h3>')+
			(lang=='en'? '<p>edit the text below, fill in your details, and push ': '<p>òøëå àú äîñîê ìîèä, îìàå àú ôøèéëí åìçöå òì ') + 
			'"' + saveLabel + '"' + 
			"");
		...
		document.write(
			...
			'</p>' + 
			'<p>' + 
			"<input accesskey='ã' type='submit' name='rewrite' value='" + saveLabel + "' onclick='onSubmitEdit(this.form)'>"+ " " + 
...
function ktov_whatsnew() {
	...
	if (/http:/.test(document.URL)) {
		...
		theText = "<p style='text-align: center; margin-left: 2cm; margin-right: 2cm; padding-top: 0.5cm; padding-bottom: 0.5cm'><a href='board.html'>äîàîøéí äàçøåðéí ùðåñôå ìàúø</a>";
	document.write(theText);
}

function ktov_mftx(current) {
	chars = "àáâãäåæçèéëìîðñòôö÷øùú"
	for (ii=10; ii<=31; ++ii) {
